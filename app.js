...
'onclick="setNoDictation(\'' + student.id + '\')">No dictation</button>' +
  '</div></td>' +
  '<td>' + criteriaHTML + '</td>' + '<td><span class="badge-grade ' + gradeObj.cssClass + '">' + gradeObj.label + '</span></td>' + '<td><input type="text" class="notes-input" value="' + escapeHtml(evalData.notes || '') + '" onchange="updateStudentNotes(\'' + student.id + '\', this.value)"></td>' + '<td style="text-align: right;">' + '<button class="btn btn-secondary student-report-btn" data-student-id="' + student.id + '">Report</button>' + '<button class="btn-icon" style="color: red;" onclick="deleteStudent(\'' + student.id + '\')"><i class="fa-solid fa-trash-can"></i></button>' + '</td>'
